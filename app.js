document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0, len = blocks.length; i < len; i++)
        ...
        (function (index) {
            ...
            blocks[i].onclick = function () {
                ...
                if (blocks[index + 7].classList.contains('spotTaken')) {
                    if (currentPlayer === 1) {
                        blocks[index].classList.add('spotTaken')
                        blocks[index].classList.add('playerOne')

                        currentPlayer = 2
                        player.innerHTML = currentPlayer
                    } else if (currentPlayer === 2) {
                        blocks[index].classList.add('spotTaken')
                        blocks[index].classList.add('playerTwo')
    ...
    function gameBoard() {
        ...
        for (let y = 0; y < winningNumbers.length; y++) {
            const block1 = blocks[winningNumbers[y][0]];
            const block2 = blocks[winningNumbers[y][1]];
            const block3 = blocks[winningNumbers[y][2]];
            const block4 = blocks[winningNumbers[y][3]];


            if (block1.classList.contains('playerOne') &&
                block2.classList.contains('playerOne') &&
                block3.classList.contains('playerOne') &&
                block4.classList.contains('playerOne')) {

                results.innerHTML = 'Player one wins!'

            }

            else if (block1.classList.contains('playerTwo') &&
                block2.classList.contains('playerTwo') &&
                block3.classList.contains('playerTwo') &&
                block4.classList.contains('playerTwo')) {
    ...
    blocks.forEach(blocks => blocks.addEventListener('click', gameBoard))
